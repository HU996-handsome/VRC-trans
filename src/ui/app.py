...
def create_app(settings, state) -> Flask:
    ...
    def _start_pipeline(settings, state):
        ...
        Combines Yakutan's non-blocking translation with MioVRC's StreamingMerger.
        """
        import sys
        import threading
        import time
        ...
        # Load hot words
        hot_words = []
        hot_words_dir = Path(__file__).resolve().parent.parent.parent / "hot_words"
        ...
        # ── Self-speech callback (Yakutan pattern: translate immediately, fire-and-forget) ──
        ...
        def on_self_speech(result: ASRResult):
            ...
            if result.is_final:
                ...
                def _do_translate():
                    ...
                    try:
                        ...
                        if state.osc_sender and settings.osc.enabled:
                            state.osc_sender.send_chatbox(translated, priority="high")
                ...
                # ── Partial result: translate immediately, send as typing indicator ──
                def _do_partial():
                    try:
                        target = settings.translation.target_language
                        ...
                        if state.osc_sender and settings.osc.enabled:
                            state.osc_sender.send_partial(translated)
                    except Exception:
                        pass

                executor.submit(_do_partial)

        # ── Others-speech callback (MioVRC pattern: StreamingMerger + dedup + self-suppression) ──
        _reverse_merger = StreamingMerger(stable_repeats=2)
        ...
        def _is_self_suppressed(text: str) -> bool:
            """Check if desktop audio result should be suppressed (MioVRC self-suppression)."""
        ...
        def _is_recent_duplicate(text: str) -> bool:
            """Check if this text was already seen recently (MioVRC dedup)."""
            now = time.time()
            norm = _normalize_for_compare(text)
            # Prune old entries
        ...
        def on_others_speech(result: ASRResult):
            ...
            if result.is_final:
                # StreamingMerger finalization
                final_text = _reverse_merger.ingest_final(text)

                # Short-result filter (MioVRC: drop < 2 chars for desktop)
                if len(final_text.strip()) < 2:
                    return

                # Self-suppression check
                if _is_self_suppressed(final_text):
                    logger.debug(f"Reverse suppressed (self): {final_text[:30]}")
                    return

                # Duplicate dedup
                if _is_recent_duplicate(final_text):
                    logger.debug(f"Reverse suppressed (dup): {final_text[:30]}")
                    return
                _recent_listen_texts.append((time.time(), _normalize_for_compare(final_text)))

                # Format: "original（translated）" with [听] prefix (MioVRC pattern)
                ...
                def _translate_reverse():
                    try:
                        ...
                        # MioVRC format: "original（translated）"
                        if translated and translated != final_text:
                            display = f"{final_text}（{translated}）"
                        else:
                            display = final_text

                        state.update_incoming(final_text, translated, is_partial=False)
                        # Don't send to VRChat chatbox - only show locally
                ...
                # Partial: use StreamingMerger for stability
                stable = _reverse_merger.ingest_partial(text)
                state.update_incoming(stable, "", is_partial=True)
        ...
        state.mic_recorder = MicRecorder(
            ...
            on_speech_end=on_mic_speech_end,
    ...
    def api_overlay_data():
        ...
        return jsonify({
            "out": snap["outgoing"]["translated"] or snap["outgoing"]["original"],
            "in": snap["incoming"]["translated"] or snap["incoming"]["original"],
            "out_partial": snap["outgoing"]["is_partial"],
            "in_partial": snap["incoming"]["is_partial"],
        })
    ...
    def api_send_text():
        ...
        if state.translator:
            ...
            if state.osc_sender and settings.osc.enabled:
                state.osc_sender.send_chatbox(processed, priority="high")
